...
When gathering repo-local context, prefer the smallest set of sources that materially sharpens the
decision analysis, but do not stop at router docs if the topic concerns shipped runtime behavior.
Read the smallest set of **actual implementation files** needed to understand the current state of
...
- ask the user to manually gather repo-local architecture or prior-research context that can be
  located locally as part of the analysis
...
3. **Findings That Merit Consideration**
   Cover only the findings that matter. For each one, explain:
   - what it is
   - why it matters
   - how strong the evidence looks
   - how it fits Mister Smith's **current implementation**
   - whether it should influence decisions now, later, or not yet

   Use `decision_horizon` if it is available. Make it clear which parts of the judgment come from
   imported evidence, code-level repo context, doc/spec context, and your own inference.
...
4. **Novelty Relative To Mister Smith**
   ...
   analysis. Use `existing_research_context` only after code-level reality is established.
