...
- [X] T032 [US5] Write audit tests in `crates/mister-smith-security/tests/audit_tests.rs` — test auth success recording (US5-AS1), authz denial recording (AS2), auth failure alert threshold (AS3), hash chain integrity and tamper detection (AS4), middleware rejection audit capture (AS5), recent_events returns limited results, max_events capacity enforced
